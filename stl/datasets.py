...
class STL10(VisionDataset):
    ...
    def __init__(
        ...
        if self.split == "train":
            ...
            print(self.data.shape,self.test_list)
            
        self.sizes = []
        files = glob.glob('../data/stl10_imgs/train_imgs/*.jpg')
        files.sort()
        for f in files:
    ...
    def select(self,sort_order, proportion=1.0):
        num_per_class = int(proportion *len(self.sizes)/10)
        print("NUM",num_per_class,self.data.shape,len(self.sizes))
        if sort_order == 'rand':
            #index = np.random.permutation(len(self.sizes))[:num_per_class*10]
            index = []
            for l in range(10):
                i = np.where(np.asarray(self.labels)==l)[0]
